	 ...
	 , _status(NET_STSTUS::STATUS_INIT)
{
     try
     {
         ...
         {
             _ssl_context.use_certificate_chain_file(ResPath::Instance()->FindResPath("ssl/server.crt"));
             _ssl_context.use_private_key_file(ResPath::Instance()->FindResPath("ssl/server.key"), Asio::ssl::context::pem);
         }
         else
         {
             _ssl_context.load_verify_file(ResPath::Instance()->FindResPath("ssl/ca.crt"));
             _ssl_context.set_verify_mode(Asio::ssl::verify_peer);

         }
		 _socket = SSLSocket(ios, _ssl_context);
...
void SSLByteStream::async_connect()
{
     ...
     {
         _timer.expires_from_now(std::chrono::duration_cast<Asio::steady_timer::duration>(_connect_timeout));
