...
{
    if (count <= 0) return;
    CHECK(_output_buffer);
    CHECK_LE(static_cast<size_t>(count), _output_buffer_size - _backed_up_bytes);
...
void BlockCompressionOutputStream::BackUp(int count)
{
    if (count <= 0) return;
    CHECK(_input_buffer);
    CHECK_LE(static_cast<size_t>(count), BLOCKSIZE - _backed_up_bytes);
    _backed_up_bytes += count;
    _byte_count -= count;
}
...
bool BlockCompressionOutputStream::Flush()
{
    ...
    CHECK_LE(compressed_size, 2*BLOCKSIZE);
