...
bool ReadBuffer::Next(const void** data, int* size)
{
    ...
    {
        CHECK_LT(_cur_pos, _cur_it->size);
...
{
    CHECK_GT(_last_bytes, 0);
    CHECK_GE(count, 0);
    CHECK_LE(count, _last_bytes);
    if (count > 0)
    {
        --_cur_it;
...
void WriteBuffer::SetData(int64_t pos, const char* data, int size)
{
    CHECK_GE(pos, 0);
    CHECK_GT(size, 0);
    CHECK_LE(pos + size, ByteCount());
    BufHandleListIterator cur_it     = _buf_list.begin();
    int                   cur_offset = 0;
    while (pos > 0)
    {
        CHECK(cur_it != _buf_list.end());
